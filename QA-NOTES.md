...
7. Footer issue. Shop / booth mean same thing to a dealer. need a universal fix as discussed elsewhere.
...
6. The footer here is the footer for a dealer as it has shop and booth. As we've discussed, these names for nav items are uniquely confusing for a dealer as the word shop has two diametrically opposite means as a noun and verb. A shop is something the dealer has and a buyer does. Please review back in the session history. you already solved this but have not implemented.
...
5. The footer here is the footer for a dealer as it has shop and booth. As we've discussed, these names for nav items are uniquely confusing for a dealer as the word shop has two diametrically opposite means as a noun and verb. A shop is something the dealer has and a buyer does. Please review back in the session history. you already solved this but have not implemented.

## feed-saved-empty
1. reorg filter as discussed earlier. 
2. The footer here is the footer for a dealer as it has shop and booth. As we've discussed, these names for nav items are uniquely confusing for a dealer as the word shop has two diametrically opposite means as a noun and verb. A shop is something the dealer has and a buyer does. Please review back in the session history. you already solved this but have not implemented.
...
3. I think theres a missed opportunity here to have your chat with the dealer mirrored on this page down below. I mean isn't this the perfect place to have the chat? 
4. no nav?
...
5. I think this is a huge missed opportunity to have the inquiries / messages from buyers on this page. This is a core functionality thing. You messages area mi
...
## messages-empty
1. Looks ok.
2. Wrong footer as discussed.
3. Buyer wouldn't have a selling tab at the top right? you can remove that whole component up there.
...
5. Same header issue. Not enough header here. The "Messages" label (which by the way needs to exist as part of that main header component. so they are in the same exact space. indented the same way etc. It can't be floating around in a random location. 
7. This needs to be sorted by object by default. this is a major functionality problem. I need you to redesign this inbox and massively prioritize the use case of the dealer trying to deal with many people trying to buy many items from him. i think one of the best ways to do this is to put all the messages related to an item on that dealers item detail page. would be helpful. But we need to get this untangled. ***** major UX issue here****
